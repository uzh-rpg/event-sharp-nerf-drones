...
from utils.events import accumulate_events
...
def render_path_interpolated(poses, poses_tms=None, num_poses=120):
    ...
    if poses_tms is None:
        poses_tms = np.linspace(0, 1, poses.shape[0])
    elif is_torch:
        poses_tms = poses_tms.cpu().numpy()

    # Note: here we ignore that sampled timestamps might coincide with "seen" timestamps
    new_tms = np.linspace(poses_tms[0], poses_tms[-1], num_poses+2)[1:-1] # exclude first and last from sampling

    # Setup Rot interpolator
    rot_interpolator = Slerp(poses_tms, R.from_matrix(poses[..., :3, :3]))
    # Query rot interpolator
    rots = rot_interpolator(new_tms).as_matrix()

    # Setup trans interpolator
    trans_interpolator = interp1d(x=poses_tms, y=poses[..., :3, 3], axis=0, kind="cubic", bounds_error=True)
    # Query trans interpolator
    trans = trans_interpolator(new_tms)

    new_poses = np.eye(4)[np.newaxis].repeat(num_poses, axis=0)
    new_poses[:, :3, :3] = rots
    new_poses[:, :3, 3] = trans

    if is_torch:
        new_poses = torch.from_numpy(new_poses).to(device)
        new_tms = torch.from_numpy(new_tms).to(device)

    if poses.shape[1] == 3:
        new_poses = new_poses[:, :3, :]

    return new_poses, new_tms
