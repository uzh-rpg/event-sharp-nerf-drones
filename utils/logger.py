...
class Logger(object):
    def __init__(self, log_dir, expname, use_wandb=False, use_tensorboard=True, wandb_id=None, args={}):
        ...
        self.is_slurm = os.getenv('SLURM_JOB_ID') is not None
        self.path = os.path.join(log_dir, expname)
        ...
        if self.use_wandb:
            ...
            self.wandb = wandb.init(project="event-sharp-nerf-drones", name=expname, config=vars(args), id=wandb_id)
            self.wandb.config.update({'save_path': self.path})
            if self.is_slurm:
                self.wandb.config.update({'slurm_job_id': os.getenv('SLURM_JOB_ID')})
                self.wandb.config.update({'slurm_cluster_name': os.getenv('SLURM_CLUSTER_NAME')})
            self.wandb.log_code(".")
        ...
        if self.use_tensorboard:
            ...
            self.tensorboard = SummaryWriter(self.path)
