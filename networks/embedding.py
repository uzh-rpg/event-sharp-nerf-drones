...
class ViewEmbedding(nn.Module):
    ...
    def __init__(self, num_embed, embed_dim, init_params="zero", **kwargs):
        ...
        # Note: nn.Embedding should be equivalent to this, but it creates a big spike
        # in the loss if used. Possibly this is due to a different random initialization
        # of the following layers: nn.Embedding uses a uniform distribution, while this, if params
        # are initialized with zeros, does not change the random generator state
        if init_params == "zero":
...
class Embedder(nn.Module):
    def __init__(self, **kwargs):
        ...
        self.n_periodic = len(self.kwargs['periodic_fns'])
        ...
        if self.kwargs['scale_pi']:
            self.freq_bands = self.freq_bands * math.pi
...
def get_embedder(multires, i=0, input_dim=3, **override_embed_kwargs):
    if i == -1:
    ...
    embed_kwargs.update(override_embed_kwargs)
