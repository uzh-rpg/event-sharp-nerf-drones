...
class CRF(nn.Module):
    ...
    def __init__(self, map_type: str, gamma: float = 2.2, init_identity=False, extra_features=0):
        ...
        assert map_type in ['none', 'gamma', 'rgb_learn', 'learn']
        self.map_type = map_type
        self.gamma = gamma
        self.extra_features = extra_features

        if 'learn' in map_type or 'rgb_learn' in map_type:
            base_features = 3 if 'rgb_learn' in map_type else 1
        ...
        if init_identity and map_type in ['learn', 'rgb_learn']:
            self.init_identity()

    def init_identity(self):
        if self.map_type not in ['learn', 'rgb_learn']:
            return
        ...
        for _ in pbar:
            ...
            ori_shape = x.shape
            x_in = x if 'rgb_learn' in self.map_type else x.reshape(-1, 1)
    ...
    def forward(self, x, x_feat=None, skip_learn=False):
        ...
        x_feat_in = x_feat
        if not skip_learn and 'learn' in self.map_type:
            ori_shape = x.shape
            x_in = x if 'rgb_learn' in self.map_type else x.reshape(-1, 1)
            if x_feat_in is not None and self.extra_features > 0:
                x_feat_in = x_feat_in.to(x_in.dtype)
                if 'rgb_learn' not in self.map_type and x_feat_in.ndim != 3:
            ...
            x_out = torch.sigmoid(res_x + x_in)
            if 'rgb_learn' not in self.map_type:
...
class TonemappingTransform(nn.Module):
    ...
    def __init__(self, map_type_rgb: str, map_type_event: str, gamma: float = 2.2, luma_standard="rec601",
                 init_learn_identity=False, extra_features_event=0, extra_features_rgb=0):
    ...
    def encode_rgb(self, x, skip_learn_crf=False, rgb_extra_feat=None, **kwargs):
        """
        Perform linear to gamma corrected and white balanced space
        """
